...
func (r *ChatRepository) UpdateThreadBinding(
	ctx context.Context,
	userFirebaseUID, projectPublicID, threadID string,
	bindingMode string,
	diagramVersionID *string,
) (*domain.Thread, error) {
	// Ensure project and thread belong to user
	...
	// Normalize binding mode
	if bindingMode == "" {
		bindingMode = domain.BindingFollowLatest
	}

	switch bindingMode {
	case domain.BindingFollowLatest:
		// Clear pinned diagram
		const q = `
update chat_threads
set binding_mode = $1,
...
returning id, title, binding_mode, pinned_diagram_version_id, created_at
`
		var t domain.Thread
		t.ProjectPublicID = projectPublicID
		if err := r.db.QueryRowContext(ctx, q, bindingMode, threadID, projectPublicID, userFirebaseUID).
			Scan(&t.ID, &t.Title, &t.BindingMode, &t.PinnedDiagramVersionID, &t.CreatedAt); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return nil, domain.ErrNotFound
			}
			return nil, err
	...
	case domain.BindingPinned:
		// Determine which diagram version to pin.
		var toPin *string
		if diagramVersionID != nil && *diagramVersionID != "" {
			// Validate the provided diagram version belongs to this user/project
			const qCheck = `
select id
...
`
			var id string
			if err := r.db.QueryRowContext(ctx, qCheck, *diagramVersionID, projectPublicID, userFirebaseUID).Scan(&id); err != nil {
				if errors.Is(err, sql.ErrNoRows) {
					return nil, domain.ErrNotFound
				}
				return nil, err
			...
			// No explicit diagram provided: use latest diagram for this project/user.
			const qLatest = `
select id
from diagram_versions
where project_public_id = $1
...
`
			var id string
			if err := r.db.QueryRowContext(ctx, qLatest, projectPublicID, userFirebaseUID).Scan(&id); err != nil {
				if errors.Is(err, sql.ErrNoRows) {
					// No diagram versions to pin.
					return nil, domain.ErrNotFound
				}
				return nil, err
			}
		...
		// If project has no current diagram and we still somehow don't have one to pin, bail.
		if toPin == nil || *toPin == "" {
			return nil, domain.ErrNotFound
		}
...
update chat_threads
set binding_mode = $1,
    pinned_diagram_version_id = $2
where id = $3
  and project_public_id = $4
  and user_firebase_uid = $5
returning id, title, binding_mode, pinned_diagram_version_id, created_at
`
		var t domain.Thread
		t.ProjectPublicID = p.PublicID
		if err := r.db.QueryRowContext(ctx, qUpdate, bindingMode, *toPin, threadID, projectPublicID, userFirebaseUID).
			Scan(&t.ID, &t.Title, &t.BindingMode, &t.PinnedDiagramVersionID, &t.CreatedAt); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return nil, domain.ErrNotFound
			}
	...
	default:
		return nil, fmt.Errorf("invalid binding_mode: %s", bindingMode)
	}
}

...
func TestScenarioCacheRepository_UpsertAndGet(t *testing.T) {
	...
	mock.ExpectQuery(`INSERT INTO simulation_scenario_cache`).
		WithArgs("dv-1", "services: []", sqlmock.AnyArg(), "", "request").
		WillReturnRows(sqlmock.NewRows([]string{"diagram_version_id", "scenario_yaml", "scenario_hash", "s3_path", "source", "created_at", "updated_at"}).
			AddRow("dv-1", "services: []", "hash1", nil, "request", now, now))

	created, err := repo.UpsertScenarioForDiagramVersion(context.Background(), "dv-1", "services: []", "request", "", false)
	require.NoError(t, err)
...
func TestScenarioCacheRepository_UpsertConflictAndOverwrite(t *testing.T) {
	...
	_, err = repo.UpsertScenarioForDiagramVersion(context.Background(), "dv-1", "new", "request", "", false)
	...
	mock.ExpectQuery(`SELECT diagram_version_id, scenario_yaml, scenario_hash`).
		...
		WillReturnRows(sqlmock.NewRows([]string{"diagram_version_id", "scenario_yaml", "scenario_hash", "s3_path", "source", "created_at", "updated_at"}).
			AddRow("dv-1", "old", "old-hash", nil, "request", now, now))
	mock.ExpectQuery(`INSERT INTO simulation_scenario_cache`).
		WithArgs("dv-1", "new", sqlmock.AnyArg(), "", "request").
		WillReturnRows(sqlmock.NewRows([]string{"diagram_version_id", "scenario_yaml", "scenario_hash", "s3_path", "source", "created_at", "updated_at"}).
			AddRow("dv-1", "new", "new-hash", nil, "request", now, now))
	_, err = repo.UpsertScenarioForDiagramVersion(context.Background(), "dv-1", "new", "request", "", true)
	require.NoError(t, err)
	require.NoError(t, mock.ExpectationsWereMet())
}
...
func TestScenarioCacheRepository_IdempotentAndScoped(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT diagram_version_id, scenario_yaml, scenario_hash`).
		WithArgs("dv-1").
		WillReturnRows(sqlmock.NewRows([]string{"diagram_version_id", "scenario_yaml", "scenario_hash", "s3_path", "source", "created_at", "updated_at"}).
			AddRow("dv-1", "same", hash, nil, "request", now, now))
	_, err = repo.UpsertScenarioForDiagramVersion(context.Background(), "dv-1", "same", "request", "", false)
	require.NoError(t, err)
